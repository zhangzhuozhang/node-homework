...
http.createServer(function (req, res) {
  ...
  if (one === 'list') {
    ...
    fs.readFile('../css/' + two, 'utf-8', function (err, data) {
      res.writeHead(200, {
    ...
    fs.readFile('../js/' + two, 'utf-8', function (err, data) {
      res.writeHead(200, {
    ...
    if (two === 'img') {
      fs.readFile('../images/img/' + three, 'binary', function (err, data) {
        res.writeHead(200, {
      ...
      fs.readFile('../images/' + two, 'binary', function (err, data) {
        res.writeHead(200, {
          "Content-Type": "image/jpeg"
        });
        res.write(data, 'binary');
  ...
  } else if (one === 'login_bg.jpg') {
    fs.readFile('../' + one, 'binary', function (err, data) {
      res.writeHead(200, {
        "Content-Type": "image/jpeg"
      });
      res.write(data, 'binary');
  ...
  else if (one === 'zhuozhuo') {
    var charter = JSON.stringify(chapterList);
    res.end(charter);
  } else if (read[0] === '/detail') {
  ...
  } else if (read[0] === "/getDetail") {
    var num = read[1].split('=')[1]
    var charte = JSON.stringify(chapterList[num - 1]);
    res.end(charte);
  } else if (url === "/login_in") {
    req.on('data', function (data) {
      var data1 = qs.parse(data.toString('utf8'), "&", null, { maxKeys: 2 });
